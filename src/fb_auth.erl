...
handle_fb_req(#httpd{method='GET'}=Req) ->
    ...
    case couch_httpd:qs_value(Req, "code") of
        undefined ->
            case couch_httpd:qs_value(Req, "token") of
                undefined ->
                ...
                Token -> 
                    ...
                    [ClientID, ClientSecret] = lists:map(fun(K) ->
                                                  case couch_config:get("fb", K, undefined) of
                                                      undefined -> throw({missing_config_value, "Cannot find key '"++K++"' in [fb] section of config"});
                                                      V -> V
                                                  end
                                              end, ["client_id", "client_secret"]),
                    handle_facebook_access_token(Req, ClientID, ClientSecret, Token)
            end;
